...
# etc. in the software we just loaded (poka.munch)
dbLoadDatabase("../../dbd/iocpokaLinux.dbd")
iocpokaLinux_registerRecordDeviceDriver(pdbbase)

### save_restore setup
< save_restore.cmd

# Access Security
dbLoadRecords("$(TOP)/pokaApp/Db/Security_Control.db","P=$(PREFIX)")
...
doAfterIocInit("registerCaputRecorderTrapListener('poka:caputRecorderCommand')")

# GUI database
dbLoadRecords("$(CAPUTRECORDER)/caputRecorderApp/Db/caputRecorder.db","P=$(PREFIX),N=300")

# second copy of GUI database
#dbLoadRecords("$(CAPUTRECORDER)/caputRecorderApp/Db/caputRecorder.db","P=pokaA:,N=300")
...
# devIocStats
dbLoadRecords("$(DEVIOCSTATS)/db/iocAdminSoft.db","IOC=poka")
...
# Arduino DC motor support
< $(TOP)/pokaApp/Db/DCMotorServer.cmd

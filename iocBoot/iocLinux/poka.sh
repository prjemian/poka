...
# Manually set IOC_STARTUP_DIR if poka.sh will reside somewhere other than iocpoka
#!IOC_STARTUP_DIR=/home/username/epics/ioc/synApps/poka/iocBoot/iocpoka
...
checkpid() {
    MY_UID=`${ID} -u`
    # The '\$' is needed in the pgrep pattern to select poka, but not poka.sh
...
start() {
	...
	screenpid
        ...
        cd ${IOC_STARTUP_DIR}
	# Run poka inside a screen session
...
run() {
	...
	screenpid
        ...
        ${ECHO} "Starting ${IOC_NAME}"
        cd ${IOC_STARTUP_DIR}
	# Run poka outside of a screen session, which is helpful for debugging
	${IOC_CMD}
    fi
}
